...
runners.forEach((full) => {
  fullNames.push(full.first_name + " " + full.last_name)
});
...
runners.map((caps) => {
firstNamesAllCaps.push(caps.first_name.toUpperCase())
})
...
runners.reduce((ticket, total) => {
  ticketPriceTotal = ticket + total.donation;
...
// Problem 1 
// The mayor will match 1 to 1 per dollar donated. Please solve this using .reduce()
let mayordonation = 2;
runners.reduce((ticket, total) => {
  ticketPriceTotal = ticket + total.donation * mayordonation;
